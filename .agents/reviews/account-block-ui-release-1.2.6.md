...
The official handoff must be the `pinova-1.2.6.zip` asset from `v1.2.6-rc2`. Before creating its tag, the publisher must verify repository-level immutability through an Administration-read, repository-scoped preflight token. It may rebuild an interrupted unpublished draft but must never alter a published Release. Handoff additionally requires the Release API to report `immutable: true`, GitHub to verify the Release and both asset attestations, and a fresh download to match the build outputs byte-for-byte.
