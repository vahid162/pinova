...
- Automated pre-release publication starts only after a successful `Quality` run on an exact `publish/vX.Y.Z-rcN` branch. The publisher validates that the commit is reachable from `main` and matches the plugin version, creates an annotated tag, builds twice, uses the GitHub CLI's draft-upload-publish sequence for the ZIP and checksum, requires native `immutable: true`, verifies both attestations, and redownloads the published assets. Never create that branch until the intended commit is already reviewed and green on `main`.
