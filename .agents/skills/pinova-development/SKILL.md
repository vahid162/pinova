...
- Treat canonical `wp-login.php` account blocking as a staged, opt-in gate. The private administrator route must remain testable while the gate is off; enable the 404 only after a logged-out private-window login succeeds and the URL is stored securely. Restrict private-route authentication and password reset to configured native-only roles, route public core login/register/lost-password helpers to Pinova, and rewrite only native administrator reset/recovery emails to the private route. Preserve the core `postpass`, nonce-protected `logout`, and keyed `confirmaction` handlers because they process password-protected posts, session termination, and privacy-request consent rather than account authentication; ensure privacy emails point to the preserved confirmation handler. Encode a nested return URL exactly once before passing it to `add_query_arg()`; WordPress does not encode newly assigned values there, and one request-query decode must restore the exact destination. Preserve `wp_login_url(..., true)` by carrying `reauth=1` to Pinova, clearing the existing authentication cookie/current user before rendering its form, and requiring fresh credentials instead of taking the logged-in shortcut. Keep `PINOVA_BLOCK_NATIVE_LOGIN=false` as an emergency recovery override, never log canonical-path probes per request, and never print the private route in public account markup or logs.
