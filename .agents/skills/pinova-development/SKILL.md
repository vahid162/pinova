...
- Automated pre-release publication starts only after a successful `Quality` run on an exact `publish/vX.Y.Z-rcN` branch. Before creating a tag, the publisher must use the repository secret `PINOVA_RELEASE_PREFLIGHT_TOKEN` (a fine-grained token limited to this repository with Administration read) to verify that native Release Immutability is enabled. It then validates that the commit is reachable from `main` and matches the plugin version, creates an annotated tag, builds twice, uses the GitHub CLI's draft-upload-publish sequence for the ZIP and checksum, requires native `immutable: true`, verifies both attestations, and redownloads the published assets. A rerun may delete and recreate only an unpublished incomplete draft; it must preserve any published Release and refuse a mutable one. Never create the publish branch until the intended commit is already reviewed and green on `main`.
