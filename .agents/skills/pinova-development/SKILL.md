...
- Automated pre-release publication starts only after a successful `Quality` run on an exact `publish/vX.Y.Z-rcN` branch. Before creating a tag, the publisher must use the repository secret `PINOVA_RELEASE_PREFLIGHT_TOKEN` (a fine-grained token limited to this repository with Administration read) to verify that native Release Immutability is enabled. It then validates that the commit is reachable from `main` and matches the plugin version, creates an annotated tag, builds twice, uses an explicit draft-upload-publish sequence for the ZIP and checksum, requires native `immutable: true`, verifies both attestations, and redownloads the published assets. A rerun may resume an unpublished draft only after its server-supplied author is `github-actions[bot]` and its name, target SHA, complete publisher-marked body, and allowed asset names match this exact release; it reuploads the reproducible assets before publication, never deletes or changes an unknown draft, preserves any published immutable Release, and refuses a published mutable one. Never create the publish branch until the intended commit is already reviewed and green on `main`.
