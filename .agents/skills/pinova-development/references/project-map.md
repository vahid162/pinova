...
- Redirects require `wp_validate_redirect()` and the allowed-host policy; pass raw return URLs to `add_query_arg()` so a single request-query decode restores the exact destination.
