...
The full-page OTP flows in `templates/login-form.php` share one hierarchy: submitted destination and edit action, code field with configured-length guidance, primary verification, resend availability, then any alternate login method. The destination is the visitor's own submitted identifier and uses bidirectional isolation; editing it returns to `authenticate` through the component state transition so short-lived OTP and reset secrets are cleared.

The resend countdown communicates availability only. While it runs, render plain status text without per-second live-region announcements; when it ends, replace that text with a touch-sized text action. Keep the OTP as one `type="text"` field with numeric input mode and `autocomplete="one-time-code"`, preserve paste and exact-length autosubmit, and never hard-code the digit count in copy. The orange verification control remains the sole primary action. The store exit remains full width but visually tertiary, and the first identifier step does not repeat that exit as an unlabeled header navigation choice.
