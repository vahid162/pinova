...
WordPress privacy export returns only the Pinova-owned mobile profile value and non-sensitive audit facts (event, UTC time, and correlation ID). It never exports OTP rows, credentials, tokens, keyed fingerprints, or arbitrary log context. Email fingerprints are trimmed and lowercased centrally before HMAC creation. Approved erasure derives email ownership from the matched account's stored address plus normalized request variants; a confirmed address without a WordPress account may erase only matching unowned pre-account records. Erasure removes matching temporary OTP records, clears personal linkage from rows owned by the User ID and unowned pre-account rows carrying the keyed email/mobile fingerprint, and deletes physical `pinova_mobile` metadata only after every identifier-dependent log batch succeeds. A failed physical-mobile lookup stops before any erasure write so the identifier remains available for retry. A matching fingerprint never authorizes anonymizing a log row owned by another User ID. For legacy pre-account records, `created_by=pinova` plus a mobile-shaped account login is an erasure-only ownership signal and is never exported as Pinova profile data. A failed OTP, metadata, or log write reports retained data and leaves the operation incomplete for retry.
