...
- Native administrator password/2FA login remains available through the private Pinova administrator route, which internally executes `wp-login.php` and its core/security-plugin hooks. Canonical account actions can return 404 only after the opt-in gate is enabled; core `action=postpass` remains available for password-protected posts. The private route remains testable before enablement, is derived from the public home URL even when WordPress core is installed in a subdirectory, and is never rendered by the public account template.
