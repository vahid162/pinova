...
Public OTP coverage must prove the provider is never called before the REST response, queued payloads are encrypted and claimed once, eligible delivery occurs through cron, decoy/native-only requests send nothing, failed delivery can retry without rotating the signed flow, expired jobs send nothing, and privacy erasure removes pending jobs. Staging acceptance must verify that WordPress cron or an external runner dispatches promptly on the target host; successful scheduling alone is not proof of delivery.
...
6. Publish a checksummed SPDX SBOM beside the ZIP and ZIP checksum. The SBOM must list each Composer package name and version from the ZIP's `vendor/composer/installed.json`; reject missing, extra, or duplicate Composer entries before attestation/tagging and again after independent redownload. Attest the ZIP with that SBOM from a workflow context bound to the release SHA, and verify that exact source digest together with native Release immutability, release and asset attestations, checksum agreement, ZIP integrity, and exact package content. Preserve any failed published candidate unchanged and correct the issue in a new candidate.
