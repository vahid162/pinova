...
- `main`: Pinova 1.2.x security line, reproducible release, structured logging, the 1.2.5 database-bootstrap hotfix, the 1.2.6 account/Blocked List and WooCommerce checkout-lifecycle corrections, and the RC4 account-brand/native-login and redirect round-trip hardening.
...
- `v1.2.6-rc4` / `0b89485`: current native-immutable 1.2.6 installation handoff. PR #15 passed all 15 Quality jobs, merged into `main`, and passed another 15-job run before `publish/v1.2.6-rc4` passed its own matrix. Publisher run `35074420994` verified reproducibility, Release and asset attestations, and its published-asset redownload. Annotated tag object `3935912` targets `0b89485`; Release `389750791` is a non-draft pre-release with `immutable: true`. An independent redownload verified installable ZIP SHA-256 `9d2e8c8c7cfee0e2491551b1806a8328cf99f2b7261e5da7326221f6c7fe45b6`. It remains a pre-release and does not itself authorize production installation or native-login gate activation.
