...
5. Before creating a tag, the publisher uses that token and the versioned GitHub API to require repository-level Release Immutability. It then validates the branch and plugin versions and confirms the SHA is reachable from `main`. It builds twice under deliberately different timezone/umask inputs, compares hashes, creates a new annotated tag, explicitly creates a draft, uploads both assets, and publishes the draft. A rerun resumes a draft only when GitHub reports `github-actions[bot]` as its author and its name, target SHA, full publisher-marked notes, and asset-name allowlist match this exact release, then reuploads the reproducible assets; an unknown draft is never changed. Published tags and Releases are never moved, deleted, or overwritten, and an existing mutable Release is a hard failure.
