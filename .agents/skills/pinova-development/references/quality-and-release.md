...
4. Before creating the publish branch, store a fine-grained token scoped only to this repository with Administration read as the Actions secret `PINOVA_RELEASE_PREFLIGHT_TOKEN`. Create `publish/vX.Y.Z-rcN` from the exact immutable commit. Its successful `Quality` run triggers `.github/workflows/publish-prerelease.yml`.
5. Before creating a tag, the publisher uses that token and the versioned GitHub API to require repository-level Release Immutability. It then validates the branch and plugin versions and confirms the SHA is reachable from `main`. It builds twice under deliberately different timezone/umask inputs, compares hashes, creates a new annotated tag, and passes both assets to `gh release create`; GitHub CLI creates the draft, uploads the assets, and then publishes it. A rerun can delete and rebuild only an unpublished incomplete draft. Published tags and Releases are never moved, deleted, or overwritten, and an existing mutable Release is a hard failure.
...
- `v1.2.6-rc2`: next native-immutable pre-release target. It must be created only after the publisher hardening is reviewed and merged, and its API/attestation checks must pass before installation handoff.
