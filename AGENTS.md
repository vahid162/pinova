...
## Current RC11 release state and blocker

As verified on 2026-09-21, [PR #30](https://github.com/vahid162/pinova/pull/30) merged the focused logout and OTP-email correction as `2bc95b061439dd2d768051cb721e16a7002c10a9`. Its exact-head PR Quality run `35520925535`, merged-main run `35521293307`, and exact `publish/v1.2.6-rc11` run `35521654794` each passed all 16 jobs. Publisher run `35522010691` built twice, published annotated tag `v1.2.6-rc11`, and passed Release plus both asset-attestation checks. Release `392490579` reports `draft:false`, `prerelease:true`, and native `immutable:true`; an independent redownload verified installable ZIP SHA-256 `111656689dfb823e94d9911b9327d9136292f0be9fcffeb532058ac9b22272fe`, ZIP integrity, its single `pinova/` root, and expected package contents. See [the RC11 release record](.agents/reviews/v1.2.6-rc11-release.md).

RC11 must not be installed. A still-open P1 review finding was confirmed after publication: WordPress may return success from `wp_safe_redirect()` when output has already committed the headers, so RC11 can still exit without a usable `Location` header or controlled fallback. The documentation follow-up's merged-main Quality run `35642055080` then exposed a second real defect: when WooCommerce replaces the checkout fragment while the modal is open, focus restoration targets the detached original opener. RC11 is immutable and remains unchanged history; the next installable candidate requires a reviewed RC12 correction for both bounded defects. Publication did not install the plugin, change a site/database/option, or authorize native-login gate activation.
...
2. Preserve RC1–RC11 and the completed PR27/PR28/PR30 chain as immutable history. RC11 is published but superseded before installation because its redirect success path does not guard `headers_sent()` and modal focus restoration can target a WooCommerce opener that has been replaced. Correct both bounded defects in a reviewed RC12 rather than editing RC11. Never substitute an implementation branch, worktree-local ZIP, or GitHub automatic source archive for the eventual release asset.
