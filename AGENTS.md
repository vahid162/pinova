...
2. Retain RC1–RC8 unchanged as publication history. Complete the post-RC8 checkout-modal correction through reviewed, exact-head green CI; if a new installable candidate is separately authorized, publish the next unused immutable RC from merged `main` and never substitute an implementation branch, worktree artifact, or GitHub automatic source archive.
