...
- `v1.2.6-rc1` is published and checksum-verified but reports `immutable: false` because it predates repository-level native Release Immutability; retain it unchanged as history. RC2–RC5 remain unchanged superseded history. `v1.2.6-rc6` is the current published 1.2.6 installation candidate: annotated tag object `2446301` targets merged commit `c327c715`, Release `389958951` reports `immutable: true`, all PR/`main`/publish/publisher gates and attestations passed, and an independent redownload verified ZIP SHA-256 `612a0ef051ea46e9a05b3f352c867b6e4704bc56bdc380c04df29ebdd7886d43`. It remains a pre-release; publication does not authorize production installation or native-login gate activation, and live desktop/mobile acceptance remains a separate post-install gate.
...
2. Retain RC1–RC5 unchanged as publication history. For the next separately authorized installation, use only the `pinova-1.2.6.zip` asset from immutable `v1.2.6-rc6`; never substitute an implementation branch, worktree artifact, or GitHub automatic source archive.
