...
- `v1.2.6-rc1` is published and checksum-verified but reports `immutable: false` because it predates repository-level native Release Immutability; retain it unchanged as history. RC2–RC5 remain unchanged superseded history. `v1.2.6-rc6` is an immutable deployed baseline: annotated tag object `2446301` targets merged commit `c327c715`, Release `389958951` reports `immutable: true`, all PR/`main`/publish/publisher gates and attestations passed, and an independent redownload verified ZIP SHA-256 `612a0ef051ea46e9a05b3f352c867b6e4704bc56bdc380c04df29ebdd7886d43`. Later read-only account acceptance found typography, spacing, focus, validation, duplicate-request, and safe-area defects, while the optional native-login gate lacked a technical activation interlock. Preserve RC6 unchanged and correct those issues in a new reviewed immutable RC before any new installation; publication never authorizes native-login gate activation.
...
2. Retain RC1–RC6 unchanged as publication history. Publish the reviewed 1.2.6 correction as the next unused immutable candidate (expected `v1.2.6-rc7`); until its tag, Release, assets, attestations, and independent redownload checks exist, it is only an implementation candidate and must not be installed.
3. Do not install an earlier RC, implementation branch, worktree artifact, or GitHub automatic source archive for a new canary. Investigate defects from a separate worktree and uniquely named disposable environment; never reuse preserved logging, RC2, or 1.2.5 verification assets without explicit authorization.
...
7. Rebuild 1.3 from corrected `main` only after the new 1.2.6 candidate completes its separately authorized staging and production canaries. Semantically port reviewed identity concepts; do not merge the obsolete branch wholesale or reuse `v1.3.0-rc1`.
