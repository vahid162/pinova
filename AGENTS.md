...
RC11 must not be installed. A still-open P1 review finding was confirmed after publication: WordPress may return success from `wp_safe_redirect()` when output has already committed the headers, so RC11 can still exit without a usable `Location` header or controlled fallback. RC11 is immutable and remains unchanged history; the next installable candidate requires a reviewed RC12 correction that checks header state before and after redirect dispatch. Publication did not install the plugin, change a site/database/option, or authorize native-login gate activation.
...
2. Preserve RC1–RC11 and the completed PR27/PR28/PR30 chain as immutable history. RC11 is published but superseded before installation because its redirect success path does not guard `headers_sent()`; correct that bounded defect in a reviewed RC12 rather than editing RC11. Never substitute an implementation branch, worktree-local ZIP, or GitHub automatic source archive for the eventual release asset.
