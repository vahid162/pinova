...
> **وضعیت انتشار:** [`v1.2.6-rc5`](https://github.com/vahid162/pinova/releases/tag/v1.2.6-rc5) اکنون pre-release تأییدشدهٔ خط امن ۱.۲.x و handoff فعلی نصب ۱.۲.۶ است. این نسخه از commit ادغام‌شدهٔ `b65370b` منتشر شده، Release آن `immutable: true` است، کنترل‌های CI و attestation موفق بوده‌اند و بازدانلود مستقل فایل [`pinova-1.2.6.zip`](https://github.com/vahid162/pinova/releases/download/v1.2.6-rc5/pinova-1.2.6.zip) مقدار SHA-256 برابر `1998a8c4c783f2624f16a650fcd3952a987e02ff9106fb585c7a9a96db3bfd36` را تأیید کرده است. RC1 تا RC4 بدون تغییر به‌عنوان سابقه باقی می‌مانند و برای نصب تازه superseded هستند. RC5 هنوز stable/latest نیست و انتشار آن به‌تنهایی مجوز نصب یا فعال‌سازی مسدودسازی مسیر بومی روی production محسوب نمی‌شود؛ ابتدا gate سلامت و پذیرش تعاملی دسکتاپ/موبایل را اجرا کنید.
...
- صفحهٔ ورود مستقل، واکنش‌گرا و RTL با پالت نارنجی/کرم برند، فرم‌های معنایی، کنترل کامل صفحه‌کلید، مسیر امن بدون جاوااسکریپت و نمایش مستقیم پیام Blocked List
- مرحلهٔ OTP با ترتیب روشن «کد ← تأیید ← ارسال دوباره ← روش جایگزین»، نمایش و ویرایش صریح شناسه، راهنمای طول کد بر پایهٔ تنظیم افزونه و شمارندهٔ متنیِ غیرقابل‌اشتباه با ورودی
...
برای جلوگیری از قفل‌شدن مدیریت، مسدودسازی مسیر اصلی در ارتقا خودکار فعال نمی‌شود:

1. مسیر خصوصی را ذخیره و در یک پنجرهٔ خصوصی مرورگر تا مرحلهٔ ورود موفق مدیر آزمایش کنید.
2. URL را در محل امن نگه دارید.
3. گزینهٔ «مسدودسازی مسیر اصلی وردپرس» را فعال کنید.
4. در یک نشست جدا تأیید کنید مسیر خصوصی کار می‌کند و ورود، ثبت‌نام و بازیابی حساب از `wp-login.php` پاسخ 404 می‌گیرد.

عملیات غیرورودی هسته فعال می‌مانند: `action=postpass` برای نوشته‌های محافظت‌شده، `action=logout` با nonce وردپرس برای خروج، `action=confirmaction` با کلید یک‌بارمصرف برای درخواست‌های حریم خصوصی، `action=confirm_admin_email` برای تأیید ایمیل مدیر و `action=exit_recovery_mode` با nonce برای خروج از Recovery Mode. reauthentication اجباری نقش‌های عادی در فرم پینوا نشست قبلی را منقضی می‌کند؛ نقش‌های ورود بومی برای این کار به مسیر خصوصی و pipeline اصلی وردپرس می‌روند. برای بازیابی اضطراری می‌توان موقتاً `PINOVA_BLOCK_NATIVE_LOGIN` را در `wp-config.php` برابر `false` قرار داد یا گزینه را با WP-CLI غیرفعال کرد. مسیر خصوصی جای رمز قوی یا 2FA نیست و نباید در صفحهٔ عمومی سایت منتشر شود.
