...
پاسخ موفقِ درخواست عمومی OTP فقط به معنی پذیرش درخواست است، نه تأیید ارسال یا دریافت کد. برای جلوگیری از افشای وجود حساب، نتیجهٔ ارسال و خطاهای کانال به کاربر اعلام نمی‌شوند؛ مدیر باید رخدادهای محدود و پاک‌سازی‌شدهٔ سرور را برای عیب‌یابی بررسی کند.

بررسی حساب و ارسال OTP در کار زمان‌بندی‌شدهٔ وردپرس انجام می‌شود تا زمان پاسخ عمومی به وضعیت حساب یا تأخیر سرویس‌دهنده وابسته نباشد. سرور به OpenSSL و WP-Cron فعال یا Cron خارجیِ منظم نیاز دارد؛ پیش از استفاده در سایت اصلی، رسیدن واقعی کد در مهلت سه‌دقیقه‌ای را در Staging آزمایش کنید.
...
پینوا رخدادهای عملیاتی را با Event code ثابت، سطح PSR-3، Correlation ID، Flow ID اختیاری برای جریان OTP، User ID اختیاری و Context محدود ثبت می‌کند. Context بر اساس Allowlist ساخته می‌شود؛ OTP، رمز، Token، Cookie، Reset key، شناسهٔ خام، متن Exception و Stack trace ذخیره نمی‌شوند.

پاسخ‌های مسیرهای REST پینوا، از جمله خطاهای اعتبارسنجی و دسترسی وردپرس، هدر `X-Pinova-Correlation-ID` دارند. رابط ورود و مدیریت شناسهٔ دریافت‌شده از سرور را کنار خطا با عنوان «کد پیگیری» نمایش می‌دهد؛ خطای شبکه بدون پاسخ سرور شناسهٔ ساختگی ندارد. این شناسه مربوط به یک درخواست است؛ Flow ID جداگانه، چند درخواست یک جریان OTP را فقط در دادهٔ امضاشدهٔ سرور و گزارش‌های عملیاتی به هم مرتبط می‌کند. در صورت خرابی جدول اختصاصی، Logger فقط ساختار ازپیش‌پاک‌سازی‌شده را به مسیرهای Fallback می‌فرستد و خرابی گزارش‌گیری نباید درخواست کاربر را متوقف کند.

رخدادهای رد خروج و نتیجهٔ بازنشانی رمز نمونه‌برداری محدود دارند و شمارندهٔ کامل درخواست‌ها نیستند: برای هر رخداد، در هر بازهٔ ۱۵ دقیقه‌ای حداکثر یک رکورد در هر خانهٔ منبع و ۱۰۰ رکورد در کل سایت ثبت می‌شود. خانه‌ها از HMAC منبع به‌دست می‌آیند؛ برخورد دو منبع ممکن است فقط یک مشاهده را حذف کند و دسترسی کاربر را تغییر نمی‌دهد. فرادادهٔ امن تأیید OTP تنها پس از بارگیری رکورد معتبر در دسترس است؛ هیچ اثرانگشتی از خود کد OTP ساخته نمی‌شود.

مدیر سایت می‌تواند گزارش‌های عملیاتی را با فیلتر دقیق ببیند و با Nonce، در بازهٔ حداکثر هفت روز و سقف ۱۰۰۰ رخداد/۲ مگابایت، خروجی رخداد دریافت کند. Context خروجی و نمایشگر از داده‌های حساس و اثرانگشت‌ها پاک می‌شود؛ تهیه‌کنندهٔ خروجی ثبت می‌شود. تغییر تنظیمات فقط با نام کلیدهای تغییرکرده ممیزی می‌شود، نه مقدارشان. بستهٔ انتشار شناسهٔ ساخت معتبر دارد، ولی Source checkout مدعی شناسهٔ انتشار نیست.
