...
سه عملیات غیرورودی هسته فعال می‌مانند: `action=postpass` برای نوشته‌های محافظت‌شده، `action=logout` با nonce وردپرس برای خروج، و `action=confirmaction` با کلید یک‌بارمصرف برای تأیید درخواست‌های حریم خصوصی. هیچ‌کدام فرم ورود حساب را نمایش نمی‌دهند. برای بازیابی اضطراری می‌توان موقتاً `PINOVA_BLOCK_NATIVE_LOGIN` را در `wp-config.php` برابر `false` قرار داد یا گزینه را با WP-CLI غیرفعال کرد. مسیر خصوصی جای رمز قوی یا 2FA نیست و نباید در صفحهٔ عمومی سایت منتشر شود.
