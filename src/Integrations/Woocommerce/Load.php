...
class Load {
	...
	public function __construct() {
		...
		if ( 'yes_redirect' === self::registration_required() ) {
			new Redirect();
		} else {
	...
	public static function is_registration_required(): bool {
		return 'no' !== self::registration_required();
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'pinova-woocommerce',
			PINOVA_URL . 'assets/js/woocommerce/checkout.js',
			[
				'jquery',
				'pinova-login-modal',
			],
	...
	public function checkout_customer_id( int $customer_id ): int {
		...
		$customer_id = UserService::create(
			$mobile->get_formatted(),
			$email,
			[
	...
	public function admin_enqueue_scripts( $hook ) {
		// This read-only parameter selects the WooCommerce admin screen assets.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$is_single_order_edit_page = in_array( $_GET['action'] ?? '', [ 'edit', 'new' ], true );
