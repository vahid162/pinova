...
final class ChangelogSyncTest extends TestCase {
	...
	public function test_equivalent_formats_are_accepted(): void {
		self::assertSame(1, ChangelogSync::assertSynchronized(self::MARKDOWN, self::README));
	}
	...
	public function test_latest_version_mismatch_is_rejected(): void {
		$this->expectException(RuntimeException::class);
		$this->expectExceptionMessage('must contain only the latest');
	...
	public function test_historical_releases_in_readme_are_rejected(): void {
		$this->expectException(RuntimeException::class);
		$this->expectExceptionMessage('must contain only the latest');

		ChangelogSync::assertSynchronized(
			self::MARKDOWN,
			self::README . "= 1.2.4 - 1405/01/01 =\n* Older change\n"
		);
	}
